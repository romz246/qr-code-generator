...
function generateQR() {
	...
	const qrType = determineQRType(qrInput);

	if (qrType === "static") {
		generateStaticQRCode(qrInput);
	} else if (qrType === "dynamic") {
		generateDynamicQRCode(qrInput);
	} else {
		alert("Invalid input for QR code generation.");
	}
}
...
function generateStaticQRCode(text) {
...
function generateDynamicQRCode(url) {
	...
	qrCodeImg.style.display = "inline";
	qrCodeImg.style.border = "2px solid #333"; // Add border to match static QR code
}
